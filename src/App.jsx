    ...
    <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", height: "100vh", background: "#f8fafc", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        ...
        <main style={{ padding: isMobile ? "20px" : "40px", paddingBottom: 100 }}>
          ...
          {view === "hardware" && <HardwareSection data={hardware} addHistory={addHistory} canEdit={canEdit} />}
...
// --- 하위 컴포넌트들 (기존과 동일하되 가독성을 위해 생략 없이 포함) ---
...
function HardwareSection({ data, addHistory, canEdit }) {
    ...
    <div>
      ...
      <ResponsiveTable 
        cols={[
          ...
          { label: "관리", render: h => canEdit && <Btn onClick={() => { setForm(h); setModal("edit"); }}>수정</Btn> }
        ]} 
        rows={data}
      />
      {modal && (
        <Modal title={modal === "add" ? "새 자산 등록" : "자산 정보 수정"} onClose={() => setModal(null)}>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <input placeholder="자산명" value={form.name || ""} onChange={e => setForm({ ...form, name: e.target.value })} style={{ padding: 12, borderRadius: 10, border: "1px solid #ddd" }} />
...
function Btn({ onClick, variant="default", children, style={}, disabled=false, type="button" }) {
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled} style={{ padding:"10px 16px", borderRadius:10, fontSize:13, fontWeight:600, cursor:"pointer", ...styles[variant], ...style }}>{children}</button>
  );
}
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead>
          <tr style={{ background: "#fbfcfd" }}>
            {cols.map((c, i) => <th key={i} style={{ padding: "16px 12px", textAlign: "left", fontSize: 12, color: "#888", borderBottom: "1px solid #f0f0f0" }}>{c.label}</th>)}
          </tr>
        </thead>
        <tbody>
          {rows.length === 0 ? <tr><td colSpan={cols.length} style={{ padding: 40, textAlign: "center" }}>{empty}</td></tr> : 
            rows.map((row, ri) => (
              <tr key={ri} style={{ borderBottom: "1px solid #f9f9f9" }}>
                {cols.map((c, ci) => <td key={ci} style={{ padding: "16px 12px", fontSize: 13 }}>{c.render ? c.render(row) : row[c.key]}</td>)}
              </tr>
            ))
          }
        </tbody>
